...
> **Reality check on the AI-visual dependency:** the pipeline is built so AI does the one thing it's genuinely reliable at for this style — **generating consistent stills** (anchors + LoRA make this a solved-enough problem in 2026). Motion is deterministic: hand-built in After Effects from those stills, so nothing generative ever touches the moving pixels (see [after_effects_workflow.md](after_effects_workflow.md) for why generative image-to-video was dropped). And because of graceful degradation (any scene can fall back to Ken Burns + text overlay, pure ffmpeg), **the worst realistic case is a Simple-History-style video of clean stills with motion overlays — still perfectly shippable.** You are not betting the channel on any single AI tool behaving perfectly.
