...
def _clip_name(scene, idx):
    """The AE render target for this scene. Uniform now that every scene is an AE
    scene; `animated_clip` (if present) is honoured for continuity."""
    return scene.get("animated_clip") or f"clips/scene_{idx:02d}.mp4"
...
def compose_motion_brief(scene, idx):
    ...
    Works for every scene — a pure camera move is just the lightest kind of AE scene."""
    facts = scene.get("visual_facts") or []
