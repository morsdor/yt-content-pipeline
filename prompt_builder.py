...
and After Effects motion briefs.

Image composition = style_card.txt prefix  +  scene_type recipe  +  the scene's subject
                    +  accent_hex highlight  +  a composition hint (from texts/motion).
Motion briefs     = the scene's animation_prompt (motion INTENT)  +  duration  +  the
                    house craft rules (transforms only, eased keys, restraint)  +  the
                    scene's visual_facts as hold-constraints.
...
Used two ways:
  1. imported by generate_images.py  → compose_image_prompt
  2. run standalone to (re)write human-readable sheets INTO the video's folder:
        python prompt_builder.py projects/001_chand_baori/storyboard.json
        # -> projects/001_chand_baori/prompts.md
        python prompt_builder.py projects/001_chand_baori/storyboard.json --motion-briefs
        # -> projects/001_chand_baori/motion_briefs.md  (the AE shot directions)
     (nothing is stored in the JSON)
"""
...
def compose_motion_brief(scene, idx):
    """One buildable After Effects shot direction, from the scene's motion intent."""
    motion = (scene.get("animation_prompt") or "slow eased push-in only").strip()
    facts = scene.get("visual_facts") or []
    clip = scene.get("animated_clip", f"clips/scene_{idx:02d}_animated.mp4")
    hold = (f" Must stay true to (may not move, deform, or be covered): "
            f"{'; '.join(facts)}." if facts else "")
    return (f"{motion}. Duration {scene['duration']}s. Build over the validated 4K still "
            f"images/scene_{idx:02d}.png — transforms only, never redraw the art; ease every "
            f"keyframe pair (F9); max 1-2 moving elements; 0.5-1s hold at both ends; render "
...
def write_prompt_sheet(sb, ctx, out_path):
    ...
    for i, s in enumerate(scenes, 1):
        ...
        if s["type"] == "animated":
            L += [f"**MOTION BRIEF → `{s['animated_clip']}`**", "```", compose_motion_brief(s, i), "```"]
        L.append("")
    Path(out_path).write_text("\n".join(L))


def motion_briefs(sb):
    """The per-scene After Effects motion briefs (animated scenes only)."""
...
def write_motion_briefs(sb, out_path):
    ...
    L = [f"# Motion briefs — {len(rows)} animated scenes (~{total}s of built motion)",
         "*Generated by prompt_builder.py from each scene's `animation_prompt` (motion intent) "
         "+ `visual_facts` (hold-constraints). One brief = one buildable AE scene, on the "
